...
from .prompts import build_system_prompt, detect_role_from_objective, DIAGNOSTIC_ROLES
...
class RLMEngine:
    ...
    def _run_one_tool(
        ...
        # Detect diagnostic role from objective for subtask/execute calls.
        _step_role = detect_role_from_objective(
            str(tc.arguments.get("objective", ""))
        ) if tc.name in ("subtask", "execute") else "lead_diagnostician"
        _step_role_label = DIAGNOSTIC_ROLES.get(_step_role, "Lead Diagnostician")
    ...
    def _apply_tool_call(
        ...
        if name == "subtask":
            ...
            _role_label = DIAGNOSTIC_ROLES.get(_role, "Lead Diagnostician")
